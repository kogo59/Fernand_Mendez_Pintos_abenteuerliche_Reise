BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Fernand_Mendez_Pintos_abenteuerliche_Reise"
TITLE_NAME = "Fernand Mendez Pinto's abenteuerliche Reise"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA_PDF) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Fernand_Mendez_Pintos_abenteuerliche_Reise_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Fernand_Mendez_Pintos_abenteuerliche_Reise_*.jpg 
